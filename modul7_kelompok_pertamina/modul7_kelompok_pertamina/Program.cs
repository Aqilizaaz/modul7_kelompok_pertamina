...
class Program
{
    public static void Main(string[] args)
    {
        ...
        {
            case "1":
                string filePath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "jurnal7_1_1201210018.json");
                DataMahasiswa_1201210018 data = new DataMahasiswa_1201210018();
                data.ReadJSON(filePath);
                break;

            case "2":
                string filePath2 = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "jurnal7_2_1201210018.json");
                DataTeamMember_1201210018 data2 = new DataTeamMember_1201210018();
                data2.ReadJSON(filePath2);
                break;

            case "3":
                string filePath3 = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "jurnal7_3_1201210018.json");
